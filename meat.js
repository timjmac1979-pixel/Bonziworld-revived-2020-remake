...
let userCommands = {
    ...
    "crosscolor": function(url) {
        if (typeof url !== 'string' || url.trim() === '') {
            this.public.crosscolor = '';
            this.room.emit('crosscolor', { guid: this.guid, url: '' });
            return;
        }
        var trimmed = url.trim();
        if (!/^https?:\/\/.+/i.test(trimmed)) {
            this.socket.emit('alert', 'Please provide a full https:// URL to your catbox image.');
            return;
        }
        this.public.crosscolor = trimmed;
        this.room.emit('crosscolor', { guid: this.guid, url: trimmed });
    },
