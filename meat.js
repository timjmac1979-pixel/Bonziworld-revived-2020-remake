...
let userCommands = {
  ...
  crosscolor: function(color) {
    ...
    var isDiscord = (clrurl.match(/cdn\.discordapp\.com/gi) || clrurl.match(/media\.discordapp\.net/gi));
    var isCatbox = clrurl.match(/files\.catbox\.moe/gi);
    var isValidExt = (clrurl.match(/\.png(\?|$)/gi) || clrurl.match(/\.jpeg(\?|$)/gi) || clrurl.match(/\.jpg(\?|$)/gi) || clrurl.match(/\.gif(\?|$)/gi) || clrurl.match(/\.webp(\?|$)/gi));
    if ((isDiscord || isCatbox) && isValidExt) {
